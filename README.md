...
### 同一 Thinking block 內 Loop

```text
保留：Loop 前內容 + 第一份循環
丟棄：第二份及後續重複、第一輪 text、第一輪 tool_use
Recovery：使用原始 messages/system/tools/tool_result 重新生成一次
最終：整理後 Thinking + Recovery Thinking + Recovery Tool Call／Final Text
```
